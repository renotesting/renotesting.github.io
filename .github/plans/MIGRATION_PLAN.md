...
1. data/users.json - Store user accounts with hashed passwords
2. data/subscribers.json - Store newsletter subscribers
3. Create utility functions for safe file I/O operations

### STEP 6: Build Authentication System
1. Registration: POST /auth/register
1.1 Validate input (email, password)
1.2 Hash password with bcryptjs
1.3 Store in users.json
1.4 Return JWT token

2. Login: POST /auth/login
2.1 Verify credentials
2.2 Generate JWT token
2.3 Return token to client

3. Protected Routes: Apply JWT middleware
3.1 Extract token from request header
3.2 Verify token signature
3.3 Allow/deny access

### STEP 7: Build Email System
1. Configure Nodemailer with Gmail or your email provider

2. Subscribe: POST /email/subscribe
2.1 Validate email format
2.2 Check if already subscribed
2.3 Save to subscribers.json

3. Send Newsletter: POST /email/send-newsletter (admin only)
3.1 Verify admin authentication
3.2 Load all subscribers
3.3 Send emails via Nodemailer
3.4 Log delivery status

### STEP 8: Create Frontend Pages
1. /login - Login page with form
2. /register - Registration page with form
3. /subscribe - Newsletter subscription form (public)
4. Update navbar to show login/logout options

### STEP 9: Update Existing Contact Form
1. Replace Formspree with your Express endpoint
2. POST /contact - Receive form submissions
...
2. Set up hosting account (Heroku, Render, Railway, etc.)
3. Configure domain migration from GitHub Pages
...
1. Outlook: Similar process, use app-specific password
2. SendGrid: API-based, requires API key
3. MailerSend: Transactional email service
...
✅ Create Node.js/Express setup with full project structure
✅ Implement authentication system (login/register)
✅ Implement email system (newsletter)
✅ Create starter code with example implementations
✅ Provide deployment guide for your chosen hosting platform